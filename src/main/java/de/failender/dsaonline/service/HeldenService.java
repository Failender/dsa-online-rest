...
public class HeldenService {
	...
	public HeldenService(HeldRepositoryService heldRepositoryService, HeldenApi heldenApi, UserRepository userRepository, VersionRepositoryService versionRepositoryService, SecurityUtils securityUtils) {
	...
	public List<Gegenstand> getInventar(BigInteger heldid) {
		...
		for(int i = 0; i<items.getChildNodes().getLength(); i++) {
			...
			Element gegenstand = (Element) node;
			int anzahl = Integer.valueOf(gegenstand.getAttribute("anzahl"));
			if(gegenstand.getChildNodes().getLength() == 0 ) {
				values.add(new Gegenstand(gegenstand.getAttribute("name"), anzahl, true));
			} else {
				for(int j = 0; j<gegenstand.getChildNodes().getLength(); j++) {
					...
					for(int k = 0; k<modallgemein.getChildNodes().getLength(); k++) {
						...
						values.add(new Gegenstand(name, anzahl, true));
						continue outer;
	...
	public List<Gegenstand> deleteItem(BigInteger heldid, int index) {
		...
		for(int i = 0; i<items.getChildNodes().getLength(); i++) {
			...
			// Disable correct removing of element from xml right now for testing
			idx ++;
		}
		try {
