...
import de.failender.dsaonline.util.AuthenticationUtil;
import de.failender.dsaonline.util.HeldenSoftwareIntegration;
...
public class HeldenService {
	...
	public void providePdfDownload(BigInteger id, int version, HttpServletResponse response) {
		HeldEntity held = heldRepositoryService.findHeld(id);
		VersionEntity versionEntity = versionRepositoryService.findVersion(held, version);
		HeldenSoftwareIntegration.provideDownload(heldenApi, new ReturnHeldPdfRequest(id, null, versionEntity.getCacheId()), response);
	}
	...
	public void provideXmlDownload(BigInteger id, int version, HttpServletResponse response) {
		...
		HeldEntity held = heldRepositoryService.findHeld(id);
		VersionEntity versionEntity = versionRepositoryService.findVersion(held, version);
		HeldenSoftwareIntegration.provideDownload(heldenApi, new ReturnHeldXmlRequest(id, null, versionEntity.getCacheId()), response);
	}
	...
	private Element getItemsElement(HeldWithVersion heldWithVersion) {
		String xml = heldenApi.request(new ReturnHeldXmlRequest(heldWithVersion.getHeld().getId(), AuthenticationUtil.getAuthentication(), heldWithVersion.getVersion().getCacheId()))
				.block();
